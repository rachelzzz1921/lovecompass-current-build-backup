#!/usr/bin/env python3
"""Validate lite + full suite flows: verify → questions → submit → result.
...
FLOW_CASES: list[dict[str, Any]] = [
    ...
    # ROS
    {"product": "ros", "suite": "s02_ros_female_lite", "code": UNIVERSAL_CODE, "gender": "female", "tier": "lite"},
    {"product": "ros", "suite": "s02_ros_male_lite", "code": UNIVERSAL_CODE, "gender": "male", "tier": "lite"},
    {"product": "ros", "suite": "s02_ros_female", "code": UNIVERSAL_CODE, "gender": "female", "tier": "full"},
    {"product": "ros", "suite": "s02_ros_male", "code": UNIVERSAL_CODE, "gender": "male", "tier": "full"},
    # MATE
    {"product": "mate", "suite": "s03_mate_female_lite", "code": UNIVERSAL_CODE, "gender": "female", "tier": "lite"},
    {"product": "mate", "suite": "s03_mate_male_lite", "code": UNIVERSAL_CODE, "gender": "male", "tier": "lite"},
    {"product": "mate", "suite": "s03_mate_female", "code": UNIVERSAL_CODE, "gender": "female", "tier": "full"},
    {"product": "mate", "suite": "s03_mate_male", "code": UNIVERSAL_CODE, "gender": "male", "tier": "full"},
]
...
def run_case(token: str, case: dict[str, Any]) -> dict[str, Any]:
    ...
    if case["code"]:
        ...
        returned = verify.get("suiteSlug")
        if returned and returned not in {suite_slug, suite_slug.replace("_lite", ""), f"{suite_slug}_lite"}:
            raise AssertionError(
                f"verify returned incompatible suite: expected ~{suite_slug}, got {returned}"
            )
    ...
    if not questions:
        raise AssertionError(f"{suite_slug} returned zero questions")
    ...
    if payload.get("suiteTier") and payload.get("suiteTier") != expected_tier:
        raise AssertionError(
            f"{suite_slug} suiteTier mismatch: expected {expected_tier}, got {payload.get('suiteTier')}"
        )
...
def main() -> None:
    print({"api": API_BASE, "cases": len(FLOW_CASES)})
    ...
    for case in FLOW_CASES:
        ...
        results.append(item)
        print(item)
    print(json.dumps({"all_suite_flows": "ok", "count": len(results)}, ensure_ascii=False))
