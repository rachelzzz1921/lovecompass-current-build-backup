...
export function RadarChart({ data, size = 280 }: { data: Axis[]; size?: number }) {
  ...
  useEffect(() => {
    ...
    grad.addColorStop(0, "rgba(140,120,240,0.45)");
    grad.addColorStop(1, "rgba(110,200,240,0.10)");
    ...
    ctx.fillStyle = grad;
    ctx.fill();
    ctx.strokeStyle = "rgba(150,140,240,0.95)";
    ctx.lineWidth = 1.6;
    ctx.stroke();
    ...
    data.forEach((d, i) => {
      ...
      ctx.fillStyle = d.color || "#a99cff";
      ctx.shadowColor = d.color || "#a99cff";
      ctx.shadowBlur = 10;
      ctx.fill();
      ctx.shadowBlur = 0;
    });
