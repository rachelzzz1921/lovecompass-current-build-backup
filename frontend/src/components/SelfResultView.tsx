...
import type { CharacterReveal, MatchType, SelfResult } from "@/data/mockResult";
    ...
    <main className="relative min-h-screen">
      ...
      <header className="relative z-10 flex items-center justify-between px-6 md:px-12 pt-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {attemptId ? (
            <Link to="/history" className="text-xs tracking-[0.25em] text-muted-foreground hover:text-foreground transition hidden md:inline">
              我的画像 →
            </Link>
          ) : null}
        </div>
              ...
              <div className="bg-glass rounded-2xl p-6 md:p-7">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className="chip chip-violet font-mono">
                    <Sparkles className="h-2.5 w-2.5" /> {reportMarkdown ? "AI" : "BASIC"}
                  </span>
                </div>
                {reportLoading && (
                  <div className="mb-4 rounded-xl border border-[oklch(0.82_0.14_200/0.25)] bg-[oklch(0.50_0.16_200/0.08)] px-4 py-3 text-sm text-foreground/75">
                    正在生成你的 AI 深度报告…
                  </div>
                )}
                {reportError && (
