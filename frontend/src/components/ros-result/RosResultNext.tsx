  ...
  return (
    <section className="space-y-7">
      ...
      <div className="rounded-2xl p-5"
        ...
        <div className="font-mono text-xl tracking-[0.25em] text-white mt-1 px-3 py-1.5 inline-block rounded-full"
          style={{ background: "rgba(99,102,241,0.12)" }}>
          {result.code}
        </div>
        <p className="text-xs text-white/55 mt-3 leading-relaxed">
          对方免费参与，做完后双人匹配报告自动解锁——两份视角，才能看见真正的你们。
        </p>
        {coupleUnlocked ? (
          <Link to="/result/ros/couple/$code" params={{ code: result.code }}
            className="inline-block mt-3 text-sm text-[#c2c4ff] underline">
            双人报告已解锁，点击查看 →
          </Link>
        ) : (
          <div className="grid grid-cols-2 gap-2 mt-4">
            <button type="button" onClick={() => copy(result.code, "关系码已复制")}
            ...
            <button type="button" onClick={() => copy(inviteUrl, "邀请链接已复制")}
