...
export function RosShareCardDialog({ result, open, onOpenChange }: Props) {
  ...
  useEffect(() => {
    ...
    drawRosSummaryShareCard(canvasRef.current, result);
    setPreviewUrl(canvasRef.current.toDataURL("image/png"));
  }, [open, result]);

  const download = useCallback(() => {
    if (!canvasRef.current) return;
    downloadCanvas(canvasRef.current, `mirror-ros-${Date.now()}.png`);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-[#12141a] border-white/10 text-white">
        <DialogHeader>
          <DialogTitle>关系画像 · 分享卡片</DialogTitle>
        </DialogHeader>
        <canvas ref={canvasRef} className="hidden" aria-hidden />
        <div className="rounded-xl overflow-hidden border border-white/10 bg-black/40">
          {previewUrl ? (
            <img src={previewUrl} alt="MIRROR ROS 分享卡片" className="w-full h-auto" />
          ) : (
